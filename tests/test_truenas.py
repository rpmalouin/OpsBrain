...
def test_collect_truenas_parses_pools(tmp_path, monkeypatch):
    ...
    creds.write_text("username=truenas_user\npassword=change_me\n")
    monkeypatch.setattr(C, "_truenas_creds", lambda cfg: ("truenas_user", "change_me"))

    def fake_get(url, user, passwd, timeout=8, max_bytes=2_000_000):
        if url.endswith("/pool"):
            return {"ok": True, "json": [{
                "name": "DATA_POOL", "status": "ONLINE", "healthy": True,
                "size": 20_000_000_000_000, "allocated": 11_000_000_000_000,
                "free": 9_000_000_000_000,
                "topology": {"data": [{"name": "raidz1-0", "type": "RAIDZ1"}]}}]}
        if url.endswith("/system/info"):
            return {"ok": True, "json": {"version": "25.10.3.1", "model": "Generic x86",
                                         "cores": 32, "physmem": 134979616768,
                                         "uptime_seconds": 86400, "hostname": "nas"}}
    ...
    assert out["pools"][0]["name"] == "DATA_POOL"
